...
const widgetData = computed(() => {
  return activeTab.value === 'ledger'
    ? [
      { title: 'TOTAL DEBITS', value: '₹1,22,249.00' },
      { title: 'TOTAL CREDITS', value: '₹1,13,808.00' },
      { title: 'CLOSING BALANCE', value: '₹8,441.00', extra: 'Dr' }
    ]
    : [
      { title: 'TOTAL BILLED', value: '₹17,98,275.00' },
      { title: 'TOTAL RECEIVED', value: '₹5,43,419.24' },
      { title: 'TOTAL DUE', value: '₹12,54,855.76' }
    ]
})
      ...
      <VCol cols="12">
        <div class="d-flex align-center gap-2">
          ...
          <VBtn @click="activeTab = 'invoices'" size="small"
            :class="activeTab === 'invoices' ? 'account_v_btn_primary' : 'account_v_btn_outlined'"
            class="account_v_btn_outlined">Invoices</VBtn>
        </div>
      </VCol>
    </VRow>
        ...
        <VCard class="account_ui_vcard account_vcard_border pa-1" title="Customer Ledger"
          ...
          <template #append>
            <div class="d-flex align-center gap-2">
              ...
              <VTextField class="accouting_field accouting_active_field" variant="outlined" type="date"
                density="compact" prepend-inner-icon="mdi-calendar" placeholder="Select date range" />
