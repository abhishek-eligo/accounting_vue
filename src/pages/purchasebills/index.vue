...
<template>
  <div>
    <VRow class="justify-center">
      <VCol cols="10">
        <VCard class="account_ui_vcard account_vcard_border" title="New Purchase Bill" subtitle="Enter the details from your vendor's bill.">
          <template #append>
            <VMenu v-model="purchaseModeMenu" location="bottom end" offset-y transition="slide-y-transition" :close-on-content-click="false">
              ...
              <VCard class="account_vcard_menu account_vcard_border">
                ...
                <div class="account_vcard_menu_items py-1">
                  <div v-for="mode in purchaseModes" :key="mode.value" class="account_vcard_menu_item" @click="selectedPurchaseMode = mode.value">
                    <div class="my-1 field_list_title cursor-pointer px-3 py-1 d-flex align-center gap-2">
                      ...
                      <span :class="selectedPurchaseMode === mode.value ? '' : 'field_list_dynamic_ml'">{{ mode.label }}</span>
                    </div>
                  </div>
                </div>
              </VCard>
          ...
          <VCardText>
            <VRow>
              ...
              <VCol cols="12" md="6">
                ...
                <VTextField v-model="vendorGSTIN" variant="outlined" readonly placeholder="15-digit GSTIN" class="accouting_field accouting_active_field" />
              </VCol>
              <VCol cols="12" md="6">
                <label class="account_label mb-2">Bill Date</label>
                <VTextField v-model="billDate" type="date" variant="outlined" class="accouting_field accouting_active_field" />
              </VCol>
              <VCol cols="12" md="6">
                <label class="account_label mb-2">Place of Supply (Vendor's State)</label>
                <VAutocomplete :items="statesList" v-model="placeOfSupply" variant="outlined" placeholder="Select state" readonly class="accouting_field accouting_active_field" />
              </VCol>
              <VCol cols="12" md="6">
                <label class="account_label mb-2">Due Date</label>
                <VTextField v-model="dueDate" type="date" variant="outlined" class="accouting_field accouting_active_field" />
              </VCol>
            </VRow>
            <VDivider class="my-4" />
            <div v-if="selectedPurchaseMode === 'both'" class="d-flex gap-2 mb-2">
              <VBtn :class="activeTab === 'inventory' ? 'account_v_btn_primary' : 'account_v_btn_outlined'" @click="activeTab = 'inventory'">Inventory Purchase</VBtn>
              <VBtn :class="activeTab === 'asset' ? 'account_v_btn_primary' : 'account_v_btn_outlined'" @click="activeTab = 'asset'">Asset Purchase</VBtn>
            </div>
            <VDataTable
            ...
            >
              <template v-slot:[`item.item`]="{ index }">
                <VTextField v-model="(activeTab === 'inventory' ? inventoryRows : assetRows)[index].item" placeholder="Item name" variant="outlined" class="accouting_field accouting_active_field" />
              </template>
              <template v-if="activeTab === 'inventory'" v-slot:[`item.account`]="{ index }">
                <VSelect v-model="inventoryRows[index].account" :items="accountsList" placeholder="Select account" variant="outlined" class="accouting_field accouting_active_field" />
              </template>
              <template v-slot:[`item.hsn`]="{ index }">
                <VTextField v-model="(activeTab === 'inventory' ? inventoryRows : assetRows)[index].hsn" placeholder="HSN Code" variant="outlined" class="accouting_field accouting_active_field" />
              </template>
              <template v-slot:[`item.qty`]="{ index }">
                <VTextField v-model="(activeTab === 'inventory' ? inventoryRows : assetRows)[index].qty" type="number" variant="outlined" class="accouting_field accouting_active_field" />
              </template>
              <template v-slot:[`item.rate`]="{ index }">
                <VTextField v-model="(activeTab === 'inventory' ? inventoryRows : assetRows)[index].rate" type="number" variant="outlined" class="accouting_field accouting_active_field" />
              </template>
              <template v-slot:[`item.gst`]="{ index }">
                <VTextField v-model="(activeTab === 'inventory' ? inventoryRows : assetRows)[index].gst" type="number" variant="outlined" class="accouting_field accouting_active_field" />
              </template>
              <template v-slot:[`item.amount`]="{ index }">
                <span>₹{{ (activeTab === 'inventory' ? inventoryRows : assetRows)[index].amount }}</span>
              </template>
              <template v-slot:[`item.actions`]="{ index }">
                <VIcon icon="mdi-trash-can-outline" class="cursor-pointer table_row_icon" :class="{ 'opacity-50': (activeTab === 'inventory' ? inventoryRows : assetRows).length === 1, 'cursor-not-allowed': (activeTab === 'inventory' ? inventoryRows : assetRows).length === 1 }" :disabled="(activeTab === 'inventory' ? inventoryRows : assetRows).length === 1" @click="removeRow(index)" />
              </template>
            </VDataTable>
            <VBtn class="account_v_btn_outlined mt-3" prepend-icon="mdi-plus-circle-outline" variant="text" @click="addRow">Add Item</VBtn>
            <VDivider class="my-4" />
            <VRow>
              <VCol cols="12">
            ...
            <VRow>
              ...
              <VCol cols="12" md="6">
                <div class="d-flex flex-column align-end">
                  <div class="d-flex justify-space-between w-100 mb-1"><span>Subtotal</span><span>₹{{ subtotal.toFixed(2) }}</span></div>
                  <div class="d-flex justify-space-between w-100 mb-1"><span>CGST</span><span>₹{{ cgst.toFixed(2) }}</span></div>
                  <div class="d-flex justify-space-between w-100 mb-1"><span>SGST</span><span>₹{{ sgst.toFixed(2) }}</span></div>
                  <div class="d-flex justify-space-between w-100 mb-1"><span>IGST</span><span>₹{{ igst.toFixed(2) }}</span></div>
                </div>
                <VDivider class="my-2" />
                <div class="d-flex justify-space-between w-100 font-weight-bold"><span>Total Amount</span><span>₹{{ totalAmount.toFixed(2) }}</span></div>
              </VCol>
            </VRow>
          </VCardText>
        </VCard>
    ...
    <v-dialog max-width="700" v-model="addVendorDialog">
      <div v-if="addVendorDialog" class="account_ui_vcard">
        <VCard title="Create a New Vendor" class="pa-2 account_vcard_border shadow-none" subtitle="Fill in the details below to add a new vendor to your records.">
          <template #append>
            <VBtn @click="addVendorDialog = false" icon="mdi-close" variant="text" size="x-small" rounded="" class="account_vcard_close_btn" />
          </template>
          <VCardText>
            <VRow>
              <VCol v-if="isVendorFieldVisible('firstName')" cols="12" lg="6" md="6">
                <label class="account_label mb-2">First Name</label>
                <VTextField v-model="newVendor.firstName" variant="outlined" density="compact" placeholder="John" class="accouting_field accouting_active_field" />
              </VCol>
              <VCol v-if="isVendorFieldVisible('lastName')" cols="12" lg="6" md="6">
                <label class="account_label mb-2">Last Name</label>
                <VTextField v-model="newVendor.lastName" variant="outlined" density="compact" placeholder="Doe" class="accouting_field accouting_active_field" />
              </VCol>
              <VCol v-if="isVendorFieldVisible('companyName')" cols="12" lg="12" md="12">
                <label class="account_label mb-2">Company Name</label>
                <VTextField v-model="newVendor.companyName" variant="outlined" density="compact" placeholder="Innovate Inc." class="accouting_field accouting_active_field" />
              </VCol>
              <VCol v-if="isVendorFieldVisible('email')" cols="12" lg="6" md="6">
                <label class="account_label mb-2">Email Address</label>
                <VTextField v-model="newVendor.email" variant="outlined" density="compact" placeholder="john.doe@example.com" class="accouting_field accouting_active_field" />
              </VCol>
              <VCol v-if="isVendorFieldVisible('phone')" cols="12" lg="6" md="6">
                <label class="account_label mb-2">Phone Number</label>
                <VTextField v-model="newVendor.phone" variant="outlined" density="compact" placeholder="(123) 456-7890" class="accouting_field accouting_active_field" />
              </VCol>
              <VCol v-if="isVendorFieldVisible('streetAddress')" cols="12" lg="12" md="12">
                <label class="account_label mb-2">Street Address</label>
                <VTextField v-model="newVendor.streetAddress" variant="outlined" density="compact" placeholder="123 Main St" class="accouting_field accouting_active_field" />
              </VCol>
              <VCol v-if="isVendorFieldVisible('city')" cols="12" lg="6" md="6">
                <label class="account_label mb-2">City</label>
                <VTextField v-model="newVendor.city" variant="outlined" density="compact" placeholder="Mumbai" class="accouting_field accouting_active_field" />
              </VCol>
              <VCol v-if="isVendorFieldVisible('state')" cols="12" lg="6" md="6">
                <label class="account_label mb-2">State</label>
                <VTextField v-model="newVendor.state" variant="outlined" density="compact" placeholder="Select a state" class="accouting_field accouting_active_field" />
              </VCol>
              <VCol v-if="isVendorFieldVisible('zipCode')" cols="12" lg="6" md="6">
                <label class="account_label mb-2">ZIP Code</label>
                <VTextField v-model="newVendor.zipCode" variant="outlined" density="compact" placeholder="400001" class="accouting_field accouting_active_field" />
              </VCol>
              <VCol v-if="isVendorFieldVisible('gstin')" cols="12" lg="6" md="6">
                <label class="account_label mb-2">GSTIN (Optional)</label>
                <VTextField v-model="newVendor.gstin" variant="outlined" density="compact" placeholder="15-digit GST Identification Number" class="accouting_field accouting_active_field" />
              </VCol>
            </VRow>
          </VCardText>
          <VCardActions class="justify-end">
            <VBtn color="success" class="account_v_btn_primary" prepend-icon="mdi-content-save" @click="saveVendor">Save Vendor</VBtn>
          </VCardActions>
        </VCard>
      </div>
    </v-dialog>
